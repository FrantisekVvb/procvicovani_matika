...
const SEED = 20250612;
const USER_NAME = 'Fiktivní uživatel – Výpočet počtu procent';
const CORRECT_STREAK_TO_LEVEL_UP = 2;
const PROBLEMS_BEFORE_RETRY = 3;
const SELECTED_MODES = ['Výpočet počtu procent'];
...
${ENGINE_STUBS}
      return {
        createPercentCountProblem,
        formatPercentCountProblemText,
        formatPercentCountDisplayHtml,
        formatDecimal,
        fromScaled,
        toScaled,
...
function formatPercentCountSessionAnswer(userAnswer, problem, formatDecimal) {
...
function simulate() {
  ...
  for (let i = 0; i < TOTAL_ANSWERS; i += 1) {
    ...
    if (dueRetry) {
      ...
      problem = percentCountProblemFromRetry(dueRetry);
    } else {
      problem = createPercentCountProblem(difficultyLevel);
    }
    ...
    results.push([
      formatPercentCountProblemText(problem),
      problem.level,
      formatPercentCountSessionAnswer(userAnswer, problem, formatDecimal),
      formatPercentCountSessionAnswer(problem.answer, problem, formatDecimal),
      shouldBeCorrect ? 1 : 0,
      formatPercentCountDisplayHtml(problem),
    ]);
  }
