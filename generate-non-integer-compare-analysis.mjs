...
const SEED = 20250606;
const USER_NAME = 'Fiktivní uživatel – porovnávání necelých čísel';
const CORRECT_STREAK_TO_LEVEL_UP = 2;
const PROBLEMS_BEFORE_RETRY = 3;
const SELECTED_MODES = ['Porovnávání necelých čísel'];
...
function loadEngine(random) {
  ...
  const body = [
    sliceBetween(mainJs, 'const NON_INTEGER_DECIMAL_MIN = 0.1;', 'const BASIC_FORM_MAX_BY_LEVEL'),
    ...
    sliceBetween(mainJs, 'function isNonIntegerCompareProblem(problem)', 'function isCompareProblem(problem)'),
    sliceBetween(mainJs, 'function getCompareOperandValue(operand)', 'function formatCompareOperand(problem, operand)'),
    'function formatCompareOperand(problem, operand) {',
    mainJs.slice(
      mainJs.indexOf('function formatCompareOperand(problem, operand)'),
      mainJs.indexOf('function formatCompareOperandHtml(problem, operand)'),
    ).replace(/^function formatCompareOperand\(problem, operand\) \{\n/, ''),
    sliceBetween(mainJs, 'function getCompareSignAnswer(left, right)', 'function buildDecimalCompareProblem'),
    sliceBetween(mainJs, 'function pickCompareOperandSign(mode', 'function randomIntegerCompareValueForSignMode'),
    ...
    sliceBetween(mainJs, 'function getArrayLcm(values)', 'function createFractionCompareSignPair'),
    sliceBetween(mainJs, 'function randomNonIntegerDecimalMagnitude()', 'function createNonIntegerDecimalTerm'),
    sliceBetween(mainJs, 'function cloneNonIntegerCompareOperand(operand)', 'function buildFractionCompareProblem({'),
...
function simulate() {
  ...
  for (let i = 0; i < TOTAL_ANSWERS; i += 1) {
    ...
    if (dueRetry) {
      ...
      problem = nonIntegerCompareProblemFromRetry(dueRetry);
    } else {
      problem = createNonIntegerCompareProblem(difficultyLevel);
    }
    ...
    if (shouldBeCorrect) {
      correctStreak += 1;
      if (correctStreak >= CORRECT_STREAK_TO_LEVEL_UP && difficultyLevel < NON_INTEGER_COMPARE_MAX_LEVEL) {
        difficultyLevel += 1;
        correctStreak = 0;
      }
    } else {
