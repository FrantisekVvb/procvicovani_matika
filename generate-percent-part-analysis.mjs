...
const SEED = 20250611;
const USER_NAME = 'Fiktivní uživatel – Výpočet části celku';
const CORRECT_STREAK_TO_LEVEL_UP = 2;
const PROBLEMS_BEFORE_RETRY = 3;
const SELECTED_MODES = ['Výpočet části celku'];
...
function loadEngine(random) {
  ...
  const body = [
    ...
    sliceBetween(mainJs, 'function isPercentPartProblem(problem)', 'function getPercentPartUserAnswer()'),
...
${ENGINE_STUBS}
      return {
        createPercentPartProblem,
        formatPercentPartProblemText,
        formatPercentPartDisplayHtml,
        formatDecimal,
        fromScaled,
        toScaled,
...
function formatPercentPartSessionAnswer(userAnswer, problem, formatDecimal) {
...
function simulate() {
  ...
  for (let i = 0; i < TOTAL_ANSWERS; i += 1) {
    ...
    if (dueRetry) {
      ...
      problem = percentPartProblemFromRetry(dueRetry);
    } else {
      problem = createPercentPartProblem(difficultyLevel);
    }
    ...
    if (shouldBeCorrect) {
      ...
      retryQueue.push({
        ...
        whole: problem.whole,
        preposition: problem.preposition,
    ...
    results.push([
      formatPercentPartProblemText(problem),
      problem.level,
      formatPercentPartSessionAnswer(userAnswer, problem, formatDecimal),
      formatPercentPartSessionAnswer(problem.answer, problem, formatDecimal),
      shouldBeCorrect ? 1 : 0,
      formatPercentPartDisplayHtml(problem),
    ]);
  }
